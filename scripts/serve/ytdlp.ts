...
// The yt-dlp bridge: `/yt?url=<link>` shells out to yt-dlp, caches the clip in
// a temp dir, and hands it back for the app's <video> path to play. yt-dlp
...
//
// Two hosts mount it, and neither owns it: the vite dev server
// (`vite-plugin-ytdlp.ts`) and the binary's `serve` command
// (`scripts/serve/main.ts`). Everything here is transport-free for that reason
// — the download, the cache and the progress readings are the same code under
// both, and each host writes only the dozen lines that turn its own request
...
export const fetching = new Map<string, FetchState>()

export const cacheKey = (url: string, secs: number): string =>
...
export const fetchClip = (url: string, secs: number): Promise<string> => {
...
// Whether yt-dlp is on PATH, asked once and remembered. Both hosts advertise
// the bridge to the app only when this comes back true, so a machine without
// yt-dlp offers no video-URL option instead of offering one that fails at the
// dialog. The version output is discarded; that the process ran at all is the
// whole answer.
let probe: Promise<boolean> | null = null

export const hasYtdlp = (): Promise<boolean> => {

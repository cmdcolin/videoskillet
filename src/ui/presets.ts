...
export const PRESETS: PresetDef[] = [
  ...
  {
    name: 'theFaceStaysOutOfIt',
    displayName: 'the face stays out of it',
    group: 'Feedback loops',
    blurb:
      'A loop that returns only its colour, behind a hue keyer that slices the live picture and is turned inside out, so the loop tints everything except one wedge of hue. The wedge sits at 123 degrees, the I axis, which is where skin lands on the wheel the subcarrier carries and why NTSC gave the I channel the wider band. A face stays a photograph while the room around it feeds back, with no mask and nothing tracking it. On the camera page a tap moves the wedge to the colour under the finger.',
    patch: {
      cfbMix: 0.88,
      cfbGain: 1.02,
      cfbDelayUs: 0.35,

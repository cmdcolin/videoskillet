...
simulate the path rather than draw the look. It ships standalone, in a browser,
and as AE / Premiere / OpenFX plugins, so Resolve, Vegas, HitFilm and Natron all
...
what you want is this look on a clip in an edit, that is the one to use.
...
hardware. Nothing in software substitutes for it. This project approaches the
...
- **A file out**: a take renders offline, faster than real time, to
  constant-framerate H.264 an editor will import
...
projects' own documentation and source, not from benchmarks run here, so nothing
